...
export type VpsPlan = {
  id: string; slug: string; name: string; description: string; cpuCores: number; ramMb: number;
...
export type VpsService = {
  id: string; planId: string; hostingLoginId: string; productName: string; description: string;
...
export function createVpsCheckout(session: CustomerSession, planId: string, operatingSystem: string) {
  return apiRequest<VpsCheckoutResponse>("/api/vps/checkout", {
    method: "POST", headers: withCustomerHeaders(session),
    body: JSON.stringify({ planId, operatingSystem, couponCode: null }),
  });
}
